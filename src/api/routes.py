...
def get_exercises_by_moduletype(module, type):
    exercises = Exercise.query.filter_by(
        module=module.upper()).filter_by(type=type.upper()).all()
    print(exercises)
    if exercises:
        if type == "fib":
            fib_exercises = [exercise.fill() for exercise in exercises]
            ...
            simple_choice_exercises = [exercise.serialize()
...
def progress_users_module(id, module):
    try:
        answers_user = AnswersUser.query.filter_by(
            user_id=id).filter_by(module=module.upper())
        answers_number = answers_user.count()
        if answers_number == 0:
...
def endpoint_mail():
    ...
    token = create_access_token(identity=email, additional_claims={"type": "password"})
...
def search_by_email(email):
    try:
        ...
        result = {"users": [], "teachers": []}
        
        if user:
            result["users"].append(user.serialize())
        elif teacher:
